...
const empty = exports.empty = () => emptyList;
const singleton = exports.singleton = head => new LinkedList(head, emptyList);
const push = exports.push = (head, tail) => new LinkedList(head, tail);
});

unwrapExports(List);
var List_1 = List.empty;
var List_2 = List.singleton;
var List_3 = List.push;

class Model {
  constructor(n, time, requestTick) {
    this.n = n;
    this.time = time;
    this.requestTick = requestTick;
  }
  static new(time = 0, n = 200, requestTick = true) {
    return new Model(n, time, requestTick);
  }
  static setTime(model, time) {
    if (model.time === time) {
      return model;
    } else {
...
const fx = model => model.requestTick ? tick(Tick) : nofx;

const view = ({ time, n }) => {
  const t = time / 1600;
  return main([styleBackground(t)], [viewControl(n), viewOrbiting(t, n), viewLemniscate(t, n)]);
};

const viewControl = n => input([id("circle-count"), type("range"), min(30), max(500), value(n), onChange(UpdateCount)]);

const Tick = time => ({ tick: time });

const UpdateCount = form({
  setCount: at(["target", "valueAsNumber"], Integer$$1)
});
...
var Moir = Object.freeze({
	...
	viewOrbiting: viewOrbiting,
	viewLemniscate: viewLemniscate
});
...
class Process$2 {
  ...
  static spawn(program, worker, node = createHost()) {
    const process = new Process$2(program, worker, node);
  ...
  receive(packet, path) {
    const payload = Process$2.toMessage(this.node, path, packet);
    if (payload) {
      this.send(payload);
    } else {
...
class Process$$1 {
  constructor({ view, update, fx }, target, renderedNode) {
    this.view = view;
    this.update = update;
    this.fx = fx;
  ...
  static spawn(program, target = window.document.body, renderedNode = createHost()) {
    const process = new Process$$1(program, target, renderedNode);
  ...
  tick(time) {
    if (this.renderedNode !== this.node) {
      patch(this.host, diff(this.renderedNode, this.node));
...
const process$1 = Process$$1.spawn(Moir, scene);
