...
const visible = visible => style$1({ visibility: visible ? "visible" : "hidden" });

const EnterKey = field("keyCode", match(13));

const onEnter = decoder => onKeyDown(or$1(and$1(EnterKey, decoder), ok$1(null)));
...
const view$1 = model => li([className(model.completed ? "completed" : model.edits ? "editing" : "")], [div([className("view")], [input([className("toggle"), type("checkbox"), checked(model.completed ? "" : null), onClick(Complete(!model.completed))]), label([onDoubleClick(Enter)], [text(model.edits || model.description)]), button([className("destroy"), onClick(Delete$1)])]), input([id(`todo-${model.id}`), className("edit"), defaultValue(model.edits), onInput(Edit), onBlur(Commit), onEnter(Commit)])]);
...
class Model {
  ...
  static addTask(model, description) {
    if (description === "") {
      ...
      return new Model([...model.tasks, newTask], model.visibility, "", model.uid + 1);
    }
  }
...
const viewEntry = inputValue => header([className("header")], [h1([], [text("todos")]), input([className("new-todo"), placeholder("What needs to be done?"), autofocus(), value(inputValue), onInput(UpdateField), onEnter(Add)])]);

const viewMain = (tasks, visibility) => section([className("main"), visible(tasks.length > 0)], [input([id("toggle-all"), className("toggle-all"), type("checkbox")]), label([For("toggle-all")], [text("Mark all as complete")]), viewTaskList(tasks, visibility)]);

const viewTaskList = (tasks, visibility) => ul([className("todo-list")], tasks.filter(task => {
  switch (visibility) {
    case "Active":
...
const viewTask = model => view$1(model).map(message => UpdateTask.new(model.id, message));
...
const viewControls = (tasks, visibility) => {
  ...
  return footer([className("footer"), visible(count > 0)], [span([className("todo-count")], [strong([], [text(`${remaining}`)]), text(`${item} left`)]), ul([className("filters")], [viewControl("", "All", visibility), viewControl("active", "Active", visibility), viewControl("completed", "Completed", visibility)]), button([className("clear-completed")], [text("Clear completed")])]);
};

const viewControl = (path, visibility, select) => {
  const selection = className(visibility === select ? "selected" : "");
  return li([onClick(ChangeVisibility(visibility))], [a([href(`#/${path}`), selection], [text(visibility)])]);
};

const view = model => section([className("todoapp")], [viewEntry(model.field), viewMain(model.tasks, model.visibility), viewControls(model.tasks, model.visibility)]);
...
const ChangeVisibility = visibility => form({
